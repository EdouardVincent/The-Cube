...
class Sprite ():
    ...
    def assign_size_coords(self):
        self.image = pygame.transform.scale(
            self.image, (Sprite.scale_image(self, self.image)))
...
button_leave_rect = button_leave.image.get_rect(
    topleft=(button_leave.x, button_leave.y))
button_tutorial_rect = button_tutorial.image.get_rect(
    topleft=(button_tutorial.x, button_tutorial.y))
button_play_rect = button_play.image.get_rect(
    topleft=(button_play.x, button_play.y))
...
button_yes_rect = Yes_button.image.get_rect(
    topleft=(Yes_button.x, Yes_button.y))
...
while game:
    ...
    while start_level:
                ...
                if button_yes_rect.collidepoint(mouse_position):
                    if lose == 1:
                        if show_score == 0:
                            ...
                            start_ticks_level = pygame.time.get_ticks()
                            seconds_level = (
                                pygame.time.get_ticks()-start_ticks_level)/1000

            if event.type == KEYDOWN:
                if event.key == K_LEFT and lose == 0:
                    nb_movements = 0
                ...
                if event.key == K_RIGHT and lose == 0:
                    nb_movements = 0
                    for x in range(len(movement)):
                        if movement[x] == 'right':
                            nb_movements += 1
        ...
        ScoreSurf = fond.render('Levels Done : %s' %
                                level, True, (255, 255, 255))
        if time_left - 5 <= 0:
            TimeLimitSurf = fond.render(
                'Time left : %s' % time_left, True, (255, 0, 0))

        else:
            TimeLimitSurf = fond.render(
                'Time left : %s' % time_left, True, (255, 255, 255))
        ...
        if show_score == 1:
            ...
            if level > file:
                ...
                CongratsSurf = fond.render(
                    'Congrats ! You beat your score !', True, (255, 255, 255))
                best_score = 1
            ...
            BestScoreSurf = fond.render(
                'Max Score : %s' % file, True, (255, 255, 255))
            ScoreSurf = fond.render('Levels Done : %s' %
                                    level, True, (255, 255, 255))
            TimeSurf = fond.render('Total Time : %s' %
                                   total_time, True, (255, 255, 255))
